...
Spring Data Elasticsearch will in version 5.0 offer Opensearch support and has the possibility to use the `RestHighLevelCLient` to connect to Opensearch clusters. 
The default dependency management stays unchanged and references Elasticsearch clients, the Opensearch dependencies are declared optional in order to not conflict with Elasticsearch ones. The dependency exclusion could be used to switch off Elasticsearch integration in favor of Opensearch.
...
<!-- Opensearch RestHighLevelClient -->
<dependency>
...
=== Using the Elasticsearch RestClient
...
To configure Opensearch integration, please check <<opensearch>> section.
